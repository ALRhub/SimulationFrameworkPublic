...
import alr_sim.controllers.Controller as ctrl
from alr_sim.controllers.TrajectoryTracking import (
...
class GymCartesianVelController(GymController):
    def __init__(
        self, robot, fixed_orientation=None, max_cart_vel=0.05, use_spline=False
    ):
        if use_spline:
            controller = GotoCartPosQuatImpedanceController(robot.dt)
        else:
            controller = CartPosQuatTrajectoryTracker(robot.dt, GotoTrajectoryLinear())
