...
import alr_sim.controllers as ctrl
import alr_sim.core.logger as logger
import alr_sim.core.Model as model
import alr_sim.core.time_keeper as time_keeper
import alr_sim.utils.geometric_transformation as geom_trans
...
class RobotBase:
    ...
    def gotoCartPositionAndQuat(
        ...
        self.gotoCartPosQuatController.executeController(self, duration, block=block)
    ...
    def beam_to_joint_pos(self, desiredJoints, run=True):
        ...
        if run:
            self.jointTrackingController.executeControllerTimeSteps(self, 1)
